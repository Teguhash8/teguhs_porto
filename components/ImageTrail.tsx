...
export default function ImageTrail({ items = [], variant = 1, children }: ImageTrailProps): JSX.Element {
    ...
    return (
        <div className="w-full h-full relative z-0 rounded-lg bg-transparent overflow-hidden" ref={containerRef}>
            {items.map((url, i) => (
                <div
            ...
            <div className="relative z-[200] pointer-events-none">
                {/* Children pass pointer events through to container unless they have pointer-events-auto */}
                {children}
            </div>
        </div>
    );
